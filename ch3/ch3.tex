...
The usage and syntax comments for quad apply here as well. In particular, if the whole last argument optimset (' 'To1X', tol) is omlued (along whth the comma that precedes it), a maximum error goal of $10^{-3}$ is assumed. Note the syntax for changing the default tolerance goal is a bit different than for quad. This is due to the fact that fminbnd has more options and is capable of doing a lot more than we will have occasion to use it for in this text. For more information, enter help optimset.
