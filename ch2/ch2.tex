...
\begin{array}{|l|l|}
\hline \begin{array}{l}
\text {subplot(m,n,i) } \rightarrow \\
\end{array} & \begin{array}{l}
\text {Causes the plot window to partition into an m x n matrix of } \\
\text {proportionally smaller subwindows, with the next plot going into }\\
\text {the ith subwindow (listed in the usual "reading order" -- left to right,  }\\
\text{ then top to bottom)}\\
...
>>subplot(2,1,2) 
>> plot(x,abs(y-p8)) , axis([- 5 5 -.000 5 .0015])
\end{lstlisting}
