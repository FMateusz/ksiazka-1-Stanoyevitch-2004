...
\begin{figure}[H]
	...
	\caption[Mesh net graph of a function f(x,y) of two variables.]{Mesh net graph of a function $f(x,y)$ of two variables.}
\end{figure}
...
\begin{figure}[H]
	...
	\caption[Two MATLAB meshgrid plots of the function]{Two MATLAB meshgrid plots of the function\\\\
...
\begin{SCfigure}[][h]
\caption[Measurement of the azimuth ang]{ Measurement of the azimuth angle $\alpha$ and the elevation angle $\varepsilon$ for use of the MATLAB command \texttt{view(azimuth, elevation)} to change perspectives on three-dimensional plots.}
...
\begin{figure}[H]
	...
	\caption[A one-dimensional rod]{A one-dimensional rod of length $L$ that is insulated along its cylindrical surface but not necessarily on the flat edges at $x=0$ and $x=L$.}
\end{figure}
...
\begin{figure}[H]
	...
	\caption[Pierre Simon de Laplace]{\label{first}Pierre Simon de Laplace (1749-1827), French mathematician and astronomer.}
\end{figure}
\columnbreak
\begin{figure}[H]
	\centering
	...
	\caption[Joseph Louis Lagrange]{\label{second}Joseph Louis Lagrange (1736-1813), French/Italian mathematician. }
\end{figure}
\columnbreak
...
\begin{figure}[H]
	...
	\caption[Dirichlet boundary conditions for the two-dimensional Laplace equation]{Dirichlet boundary conditions for the two-dimensional Laplace equation $u_{x x}+u_{y y}=0$ are specified on each of the four sides making up the boundary of a rectangular domain in the plane. If the four functions specified on each edge are continuous, then they give a well-posed problem for the Laplace equation.}
\end{figure}
...
\begin{SCfigure}[][h]
\caption[Illustration of the stencil for the linear equation]{Illustration of the stencil for the linear equation (16)\protect\footnotemark ~  These grid values form a cross centered at the $u_{i, j}$-grid value. Although the central grid value $u_{i, j}$ will be at a point interior to the rectangle, one or two of the other four grid values may be from boundary points.}
...
\begin{multicols}{2}
	...
	\caption[Spy plot of the coefficient matrix A]{Spy plot of the coefficient matrix $A$ for Example 11.6. The $n z=148$ indicates the number of nonzero entries of the 1296 entries of $A$. We know the diagonal entries all equal 4 and all other nonzero entries equal $-1$. Such spy plots make the general structure of such matrices quite evident.} 
	\end{figure}
\columnbreak
...
\caption[Illustration of the unit outward normal vector at a boundary point (x, y) of a planar domain .]{Illustration of the unit outward normal vector at a boundary point $(x, y)$ of a planar domain $\Omega$.}
\end{figure}
...
\caption[A grid for the Neumann problem]{A grid for the Neumann problem of Example 11.7. The solid-labeled nodes are the ones (function values) that need to be solved for; the hollow nodes are the ghost nodes needed to set up the method.}
\end{figure}
\columnbreak
